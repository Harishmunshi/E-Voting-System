    ...
    return (
      <main className="min-h-screen">
        ...
        <Card className="mx-auto mt-16 max-w-xl p-8 text-center">
          <h2 className="text-2xl font-bold text-slate-950">Results are not published yet</h2>
  ...
  return (
    <main className="min-h-screen">
      ...
      <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <h2 className="text-3xl font-bold text-slate-950">Student Council Election Results</h2>
          ...
          {dashboard.positions.map((position) => {
            ...
            return (
              <Card key={position.id} className="p-5">
                <p className="text-sm font-semibold uppercase tracking-[0.16em] text-gold-700">{position.title}</p>
                <h3 className="mt-2 text-2xl font-bold text-royal-900">{winner?.name ?? "No winner"}</h3>
                  ...
                  {candidates.map((candidate) => (
                    <div key={candidate.id}>
                      ...
                      <div className="mt-1 h-2 rounded-full bg-slate-100">
                        <div className="h-2 rounded-full bg-royal-700" style={{ width: `${percentage(candidate.vote_count ?? 0, total)}%` }} />
                      </div>
                    </div>
                  ))}
                </div>
